...
from preset_maps import RULES_TEXT, ALTAR_MAP, MOSAIC_MAP, PRISON_MAP, RIVER_MAP, CREEK_MAP
...
async def api_generate_map(payload: dict[str, Any]):
    ...
    if map_type == "Creek":
        map_data = eng.MapData(CREEK_MAP["width"], CREEK_MAP["height"], [row[:] for row in CREEK_MAP["grid"]])
    elif map_type == "River":
